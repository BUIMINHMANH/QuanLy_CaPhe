...
namespace project2.DataAccess
{
    class ChiTietPhieuXuatDAL
    {
        ...
        public DataTable LayThongTinPhieuXuat(string mapx)
        {
            string query = "select px.Mapx, ctpx.Masp from PhieuXuat px inner join ChiTietPhieuXuat ctpx on px.Mapx = ctpx.Mapx inner join SanPham sp on ctpx.Masp = sp.Masp where px.Mapx = '" + mapx + "'";
            return dah.get_DaTaTable(query);
        }
